...
with col1:
	...
	# Отображение данных
	rainy_days = filtered_data["is_rainy"].sum()
	avg_temp = filtered_data["avg_temp"].mean()
	avg_comfort = filtered_data["comfort_index"].mean()

	col1_1, col1_2, col1_3 = st.columns(3)
	col1_1.metric("Средняя температура", f"{avg_temp:.1f}°C")
	col1_2.metric("Дождливых дней", int(rainy_days))
	col1_3.metric("Средний комфорт", f"{avg_comfort:.1f}")
...
with col2:
	...
	if selected_graph == 'Сравнение погодных показателей':
		...
		st.plotly_chart(fig)
		fig = px.line(data_frame=filtered_data, 
                	x='date', y='comfort_index', 
                 	color='city',
                    title='Индекс комфорта (чем выше — тем комфортнее)',
